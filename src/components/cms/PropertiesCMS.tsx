...
const PropertiesCMS = () => {
  ...
  const updatePropertyFeatures = (id, features) => {
    setProperties(properties.map(property => 
      property.id === id ? { ...property, features: features.split(',').map(f => f.trim()) } : property
    ));
  };
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-semibold">Properties ({properties.length})</h3>
          <div className="flex gap-2">
            <Badge variant="secondary" className="bg-green-100 text-green-800">
              {properties.filter(p => p.type === "Completed").length} Completed
            </Badge>
            ...
            <Badge variant="secondary" className="bg-purple-100 text-purple-800">
              {properties.filter(p => p.type === "Open Plots").length} Open Plots
            </Badge>
          </div>
        </div>
                ...
                <div className="flex-grow space-y-3">
                  ...
                  {isEditing ? (
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="col-span-2">
                        <Label htmlFor={`features-${property.id}`}>Features (comma separated)</Label>
                        <Input
                          id={`features-${property.id}`}
                          value={property.features.join(', ')}
                          onChange={(e) => updatePropertyFeatures(property.id, e.target.value)}
                        />
                      </div>
                      <div className="col-span-2">
                        <Label htmlFor={`image-${property.id}`}>Image URL</Label>
                        <div className="flex gap-2">
                          <Input
                            id={`image-${property.id}`}
                            value={property.image}
                            onChange={(e) => updateProperty(property.id, 'image', e.target.value)}
                          />
                          <Button variant="outline" size="sm">
