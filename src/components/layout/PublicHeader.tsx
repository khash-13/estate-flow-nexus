      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center justify-between py-4">
          <Link to="/public" className="flex items-center space-x-2">
            ...
            <span className="text-2xl font-bold text-estate-navy">EstateCorp</span>
          </Link>
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {navigation.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className={`text-sm font-medium transition-colors hover:text-estate-navy ${
                  isActive(item.href) ? "text-estate-navy" : "text-gray-700"
                }`}
              >
                {item.name}
              </Link>
